function varargout = select_ROI_gui2(varargin)
% SELECT_ROI_GUI2 MATLAB code for select_ROI_gui2.fig
%      SELECT_ROI_GUI2, by itself, creates a new SELECT_ROI_GUI2 or raises the existing
%      singleton*.
%
%      H = SELECT_ROI_GUI2 returns the handle to a new SELECT_ROI_GUI2 or the handle to
%      the existing singleton*.
%
%      SELECT_ROI_GUI2('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SELECT_ROI_GUI2.M with the given input arguments.
%
%      SELECT_ROI_GUI2('Property','Value',...) creates a new SELECT_ROI_GUI2 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before select_ROI_gui2_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to select_ROI_gui2_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help select_ROI_gui2

% Last Modified by GUIDE v2.5 30-Jun-2016 14:57:19

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @select_ROI_gui2_OpeningFcn, ...
                   'gui_OutputFcn',  @select_ROI_gui2_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before select_ROI_gui2 is made visible.
function select_ROI_gui2_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to select_ROI_gui2 (see VARARGIN)

% Choose default command line output for select_ROI_gui2
...
% UIWAIT makes select_ROI_gui2 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = select_ROI_gui2_OutputFcn(hObject, eventdata, handles) 
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global ROI_handles SnapPathName StackFileName StackPathName bkg_curr_val;
...
%         ROI_handles = {};
        ...
        axes(handles.image_axes)
        bkgmapH = imshow(imadjust(uint16(tiffStack(:,:,1))));
        ...
        save([path file] , 'bkg' , '-append')
        pos = h.getPosition();
        textX = 12+(max(pos(: , 1)) + min(pos(: , 1)))/2;
        textY = (max(pos(: , 2)) + min(pos(: , 2)))/2;  
        text('position',[textX textY],'fontsize',20 , 'Parent' , ...
                parentbkgmapH , 'Color' , 'w' ,'string','Background');
...
tiffStack = tiffStackReaderFast([StackPathName StackFileName]);
warning('on' , 'all');
